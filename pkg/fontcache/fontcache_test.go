...
func TestFontCacheParsing(t *testing.T) {
	...
	_, _, _, _, err := GetFonts(mockRegular, mockBold, nil, nil)
...
func TestFontCacheReuse(t *testing.T) {
	...
	GetFonts(mockBytes, mockBytes, nil, nil)

	// Second call should return cached result (same error)
	_, _, _, _, err1 := GetFonts(mockBytes, mockBytes, nil, nil)
	_, _, _, _, err2 := GetFonts(mockBytes, mockBytes, nil, nil)
